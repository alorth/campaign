    ...
    public function fire()
    {
        ...
        if ( ! Schema::connection($connection)->hasTable('goals'))
        {
            Schema::connection($connection)->create('goals', function($table)
            {
                ...
                $table->integer('isDesktop')->unsigned()->default(0);
                $table->integer('isMobile')->unsigned()->default(0);
                $table->integer('isTablet')->unsigned()->default(0);
                $table->integer('isIphone')->unsigned()->default(0);
                $table->integer('isAndroidOS')->unsigned()->default(0);
                $table->primary(array('name', 'experiment'));
            });
        }

        // Create logs table.
        if ( ! Schema::connection($connection)->hasTable('logs'))
        {
            Schema::connection($connection)->create('logs', function($table)
            {
                $table->increments('id');
                $table->string('experiment');
                $table->string('goal');
